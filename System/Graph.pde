public class Graph{
  ...
  public void displayIntegral(){
    ...
    inputSys.display();
    for (Point p: Points){
      totalArea += p.displayIntegral(camera, 0.05);
    }
    fill(0,0,0);
    text("Estimate of Total Area under Curve ~ " + totalArea, width - 230, height-5);
  }
  ...
  public void displayAxis(){
    ...
    for (Point point: ranges){
     ...
     if (Dimension == 3){
       for (int i = 0; i < 6; i++){
         text(10*(i%2*2-1),ranges.get(i).getCoords2D().x*width/cameraMagnitude+width/2,ranges.get(i).getCoords2D().y*-height/cameraMagnitude+height/2);
       }
     }
     else{
       for (int i = 2; i < 6; i++){
         text(-10*(2*(ceil(abs(i-3.5))-1)-1),ranges.get(i).getCoords2D().x*width/cameraMagnitude+width/2,ranges.get(i).getCoords2D().y*-height/cameraMagnitude+height/2);
       }
     }
  }
