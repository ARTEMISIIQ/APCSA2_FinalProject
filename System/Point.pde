public class Point{
 ...
 public PVector getCoords3D(){ // Coords in 3D space
  return this.coordinates3D; 
 }
 
 public PVector getCoords2D(){ // Coords after projection
 ...
 public void display(Camera c){ // Appear in the desired location on the screen with unit vectors
 ...
 public void displayVector(Camera c){ // The slope of each line is equal to the function applied at the x and y values.
   float cameraMagnitude = c.getMag();
   float d = c.getVector().dist(coordinates3D);
 ...
 public PVector vectorFunction(float x, float y){ // Applies the function on the coords of the point
   float a = inputSys.function(x,y,0);
   if (a == Integer.MAX_VALUE){
     return new PVector(0,0);
   }
   return new PVector(1,a).setMag(200 / cameraMagnitude);
 }
 
 public float displayIntegral(Camera c, float m){ // Makes a rectangle from the point to the x-axis
   float cameraMagnitude = c.getMag();
   float d = c.getVector().dist(coordinates3D);
   strokeWeight(1);
   ...
   return m * coordinates2D.y; // return SIGNED area of the rectangle (m = width, coordinates2D.y = height)
 }
}
